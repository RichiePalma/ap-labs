...
// Reference: https://en.wikibooks.org/wiki/Algorithm_Implementation/Miscellaneous/Base64#C
...
static void progress(int task) {
	unsigned long percentage = (byteswritten * 100) / size;
    if(task == 0){
   ...
   if (strcmp(argv[1], "--encode") == 0) {
		...
		*buffwritesize = 3;
        ...
        if(strcmp(argv[1],"encoded.txt")){
            warnf("File to decode is not default encoded.txt created by the encode task, it may haven not been decoded correctly.\n");
        }
        infof("Decode task finished and successfully written in decoded.txt\n");
	}
    else {
